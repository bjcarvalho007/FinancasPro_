import { jsPDF } from 'jspdf';
import { Transaction } from '../types';

interface ReportData {
  transactions: Transaction[];
  baseIncome: number;
  baseBalance: number;
  currentCurrency: 'BRL' | 'USD' | 'EUR';
  userEmail: string;
}

// Map currency symbol
export const getCurrencySymbol = (currency: 'BRL' | 'USD' | 'EUR'): string => {
  const map = {
    BRL: 'R$',
    USD: '$',
    EUR: '€',
  };
  return map[currency] || 'R$';
};

// Map currency formatter
export const formatCurrency = (val: number, currency: 'BRL' | 'USD' | 'EUR'): string => {
  const locale = currency === 'BRL' ? 'pt-BR' : currency === 'USD' ? 'en-US' : 'de-DE';
  return new Intl.NumberFormat(locale, { style: 'currency', currency }).format(val);
};

// Translate transaction types for report readability
const getTypeLabel = (type: string) => {
  switch (type) {
    case 'fixos': return 'Despesa Fixa';
    case 'variaveis': return 'Despesa Variável';
    case 'parcelas': return 'Compra Parcelada';
    default: return type;
  }
};

/**
 * EXPORT 1: Premium PDF report using jsPDF custom drawing for pristine styling and precision layout.
 */
export const exportPremiumPDF = ({
  transactions,
  baseIncome,
  baseBalance,
  currentCurrency,
  userEmail,
}: ReportData) => {
  const doc = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4',
  });

  const totalInflow = baseIncome + baseBalance;
  const totalSpent = transactions.reduce((sum, t) => sum + t.amount, 0);
  const totalPaid = transactions.reduce((sum, t) => sum + (t.paid_amount || 0), 0);
  const totalPending = transactions.reduce((sum, t) => sum + (t.amount - (t.paid_amount || 0)), 0);
  const netBalance = totalInflow - totalSpent;

  // Track dynamic cursor positions
  let y = 15;
  let pageNum = 1;

  // Add customized premium font helpers & header
  const drawPageBorderAndFooter = () => {
    // Add page outline or top progress line (Premium accent color)
    doc.setDrawColor(79, 70, 229); // indigo-600
    doc.setLineWidth(1.5);
    doc.line(10, 8, 200, 8); // Deep top ribbon

    // Bottom footer area
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(100, 116, 139); // slate-500
    doc.text(
      `FinançasPro Premium • Relatório Gerencial consolidado de segurança extrema`,
      15,
      287
    );
    doc.text(`Pág. ${pageNum}`, 190, 287, { align: 'right' });
  };

  const checkPageOverflow = (neededHeight: number) => {
    if (y + neededHeight > 275) {
      doc.addPage();
      pageNum++;
      y = 20;
      drawPageBorderAndFooter();
    }
  };

  // Draw initial borders
  drawPageBorderAndFooter();

  // Draw branding logo box
  doc.setFillColor(15, 23, 42); // slate-900 background
  doc.rect(12, 12, 186, 26, 'F');

  // Decorative vector inside logo box
  doc.setFillColor(16, 185, 129); // emerald-500
  doc.rect(18, 18, 2.5, 14, 'F');
  doc.setFillColor(99, 102, 241); // indigo-500
  doc.rect(22, 21, 2.5, 11, 'F');

  // Title texts inside box
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(15);
  doc.text('FINANÇAS PRO', 28, 23);
  
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(148, 163, 184); // slate-400
  doc.text('AUDITORIA FINANCEIRA & DEMONSTRATIVO CONSOLIDADO', 28, 28);
  doc.text(`USUÁRIO: ${userEmail.toUpperCase()}`, 28, 33);

  // Current Date tag inside logo box
  doc.setFillColor(30, 41, 59); // slate-800
  doc.roundedRect(145, 17, 48, 16, 2, 2, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.text('GERADO EM:', 149, 23);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(16, 185, 129); // emerald-400
  doc.text(new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }), 149, 28);

  y = 48;

  // 1. Executive Summary Panel
  doc.setTextColor(15, 23, 42); // slate-900
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('1. SUMÁRIO DA COMPOSIÇÃO DE CAIXA', 12, y);
  y += 4;

  // Draw border line
  doc.setDrawColor(226, 232, 240); // slate-200
  doc.setLineWidth(0.5);
  doc.line(12, y, 198, y);
  y += 4;

  // 3-Column stats grid
  // Col 1: Liquid available (base income + starting balance)
  doc.setFillColor(248, 250, 252); // soft slate
  doc.roundedRect(12, y, 58, 24, 2, 2, 'F');
  doc.setDrawColor(226, 232, 240);
  doc.roundedRect(12, y, 58, 24, 2, 2, 'D');

  doc.setTextColor(71, 85, 105); // slate-600
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.text('DISPONÍVEL INICIAL (A)', 16, y + 6);
  doc.setTextColor(15, 23, 42);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11.5);
  doc.text(formatCurrency(totalInflow, currentCurrency), 16, y + 14);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.5);
  doc.setTextColor(100, 116, 139);
  doc.text(`Salário base + Balanço cadastrado`, 16, y + 20);

  // Col 2: Total Spent
  doc.setFillColor(248, 250, 252);
  doc.roundedRect(76, y, 58, 24, 2, 2, 'F');
  doc.setDrawColor(226, 232, 240);
  doc.roundedRect(76, y, 58, 24, 2, 2, 'D');

  doc.setTextColor(71, 85, 105);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.text('DESPESAS GERAIS (B)', 80, y + 6);
  doc.setTextColor(239, 68, 68); // Red
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11.5);
  doc.text(formatCurrency(totalSpent, currentCurrency), 80, y + 14);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.5);
  doc.setTextColor(100, 116, 139);
  doc.text(`Relação de passivos cadastrados`, 80, y + 20);

  // Col 3: Balance leftovers
  const balanceColor = netBalance >= 0 ? [16, 185, 129] : [239, 68, 68];
  doc.setFillColor(248, 250, 252);
  doc.roundedRect(140, y, 58, 24, 2, 2, 'F');
  doc.setDrawColor(226, 232, 240);
  doc.roundedRect(140, y, 58, 24, 2, 2, 'D');

  doc.setTextColor(71, 85, 105);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.text('SALDO PROJETADO (A - B)', 144, y + 6);
  doc.setTextColor(balanceColor[0], balanceColor[1], balanceColor[2]);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11.5);
  doc.text(formatCurrency(netBalance, currentCurrency), 144, y + 14);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.5);
  doc.setTextColor(100, 116, 139);
  doc.text(`Líquido livre projetado no mês`, 144, y + 20);

  y += 29;

  // Let's add extra detailed sub-metrics about settlements (Total Pago vs Pendente)
  doc.setFillColor(254, 242, 242); // very soft red
  doc.roundedRect(12, y, 88, 14, 2, 2, 'F');
  doc.setTextColor(185, 28, 28);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.text('TOTAL DE VALORES EM ABERTO:', 16, y + 9);
  doc.setFontSize(10.5);
  doc.text(formatCurrency(totalPending, currentCurrency), 95, y + 9.5, { align: 'right' });

  doc.setFillColor(240, 253, 250); // very soft green
  doc.roundedRect(110, y, 88, 14, 2, 2, 'F');
  doc.setTextColor(15, 118, 110);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.text('BAIXAS JÁ COMPROVADAS:', 114, y + 9);
  doc.setFontSize(10.5);
  doc.text(formatCurrency(totalPaid, currentCurrency), 193, y + 9.5, { align: 'right' });

  y += 19;

  // 2. Category Distribution summary
  doc.setTextColor(15, 23, 42);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('2. DISTRIBUIÇÃO CONSOLIDADA POR CATEGORIA', 12, y);
  y += 4;
  doc.setTextColor(226, 232, 240);
  doc.line(12, y, 198, y);
  y += 4;

  // Compute category breakdown
  const catSummaryMap: Record<string, { total: number; paid: number; count: number }> = {};
  transactions.forEach(t => {
    const c = t.cat || 'Outros';
    if (!catSummaryMap[c]) {
      catSummaryMap[c] = { total: 0, paid: 0, count: 0 };
    }
    catSummaryMap[c].total += t.amount;
    catSummaryMap[c].paid += (t.paid_amount || 0);
    catSummaryMap[c].count += 1;
  });

  const categories = Object.keys(catSummaryMap).sort((a, b) => catSummaryMap[b].total - catSummaryMap[a].total);

  if (categories.length === 0) {
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(9);
    doc.text('Nenhuma movimentação de débito ou categoria encontrada.', 16, y + 5);
    y += 10;
  } else {
    // We will render categories in a side-by-side or listing block
    const itemsPerCol = Math.ceil(categories.length / 2);
    const colWidth = 92;

    doc.setFontSize(8);
    for (let index = 0; index < categories.length; index++) {
      const cat = categories[index];
      const data = catSummaryMap[cat];
      const isCol2 = index >= itemsPerCol;
      const colX = isCol2 ? 110 : 12;
      const rowOffset = isCol2 ? (index - itemsPerCol) * 10 : index * 10;
      
      checkPageOverflow(rowOffset + 12);
      const currentY = y + rowOffset;

      // Category rounded indicator
      doc.setFillColor(241, 245, 249);
      doc.roundedRect(colX, currentY, colWidth, 8, 1, 1, 'F');

      doc.setTextColor(15, 23, 42);
      doc.setFont('helvetica', 'bold');
      const formattedLabel = cat.charAt(0).toUpperCase() + cat.slice(1);
      doc.text(`${formattedLabel} (${data.count}x)`, colX + 3, currentY + 5.5);

      doc.setFont('helvetica', 'normal');
      doc.setTextColor(71, 85, 105);
      doc.text(`Pendente: ${formatCurrency(data.total - data.paid, currentCurrency)}`, colX + 45, currentY + 5.5);

      doc.setFont('helvetica', 'bold');
      doc.setTextColor(15, 23, 42);
      doc.text(formatCurrency(data.total, currentCurrency), colX + colWidth - 3, currentY + 5.5, { align: 'right' });
    }
    y += itemsPerCol * 10 + 6;
  }

  y += 2;

  // 3. Transactions spreadsheet details table
  checkPageOverflow(30);
  doc.setTextColor(15, 23, 42);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('3. FLUXO DETALHADO DO LIVRO-RAIZ', 12, y);
  y += 4;
  doc.setTextColor(226, 232, 240);
  doc.line(12, y, 198, y);
  y += 4;

  // Table Headers
  doc.setFillColor(15, 23, 42); // slate-900 heading row
  doc.rect(12, y, 186, 7.5, 'F');
  
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7.5);
  doc.text('VENCIMENTO', 15, y + 5);
  doc.text('LANÇAMENTO', 42, y + 5);
  doc.text('CATEGORIA', 95, y + 5);
  doc.text('FLUXO', 125, y + 5);
  doc.text('STATUS', 155, y + 5);
  doc.text('VALOR TOTAL', 195, y + 5, { align: 'right' });

  y += 7.5;

  // Standard sorted transaction rendering
  const sortedTx = [...transactions].sort((a, b) => {
    return (a.due || '').localeCompare(b.due || '');
  });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);

  sortedTx.forEach((tx, idx) => {
    checkPageOverflow(11);

    // Zebra striping for ultra high contrast professional view
    if (idx % 2 === 0) {
      doc.setFillColor(248, 250, 252);
      doc.rect(12, y, 186, 8.5, 'F');
    } else {
      doc.setFillColor(255, 255, 255);
      doc.rect(12, y, 186, 8.5, 'F');
    }

    // Grid divider row border
    doc.setDrawColor(241, 245, 249);
    doc.setLineWidth(0.3);
    doc.line(12, y + 8.5, 198, y + 8.5);

    // Value alignments
    doc.setTextColor(71, 85, 105);
    const dateFormatted = tx.due ? tx.due.split('-').reverse().join('/') : '—';
    doc.text(dateFormatted, 15, y + 5.5);

    doc.setTextColor(15, 23, 42);
    doc.setFont('helvetica', 'bold');
    doc.text(tx.name.length > 28 ? tx.name.substring(0, 26) + '...' : tx.name, 42, y + 5.5);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 116, 139);
    const catLabel = tx.cat ? tx.cat.charAt(0).toUpperCase() + tx.cat.slice(1) : 'Outros';
    doc.text(catLabel, 95, y + 5.5);

    doc.text(getTypeLabel(tx.type), 125, y + 5.5);

    // Status formatted color pills
    const remaining = tx.amount - (tx.paid_amount || 0);
    if (remaining <= 0) {
      doc.setTextColor(15, 118, 110); // emerald-700
      doc.setFont('helvetica', 'bold');
      doc.text('PAGO', 155, y + 5.5);
    } else if (tx.paid_amount > 0) {
      doc.setTextColor(180, 83, 9); // amber-700
      doc.setFont('helvetica', 'bold');
      doc.text('PARCIAL', 155, y + 5.5);
    } else {
      doc.setTextColor(185, 28, 28); // rose-750
      doc.setFont('helvetica', 'bold');
      doc.text('PENDENTE', 155, y + 5.5);
    }

    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 23, 42);
    doc.text(formatCurrency(tx.amount, currentCurrency), 195, y + 5.5, { align: 'right' });

    y += 8.5;
  });

  // End of report signature validation area
  checkPageOverflow(30);
  y += 8;
  doc.setDrawColor(203, 213, 225); // slate-300
  doc.setLineWidth(0.65);
  doc.line(12, y, 198, y);
  y += 5;

  doc.setFont('helvetica', 'italic');
  doc.setFontSize(7.5);
  doc.setTextColor(100, 116, 139);
  doc.text(
    `Declaração de autenticidade: Este demonstrativo espelha com integridade as informações armazenadas localmente e sincronizadas online no Firebase Database. Chave única de integridade de sessão ativa: md5_session_v2_${idxGenerator(9)}`,
    12,
    y + 2
  );

  // Trigger browser download action safely
  const fileDate = new Date().toISOString().split('T')[0];
  doc.save(`FinancasPro_Demonstrativo_Premium_${fileDate}.pdf`);
};

/**
 * EXPORT 2: Corporate-Level Spreadsheet Export. Generates clean UTF-8 encoded, 
 * beautifully styled CSV reports easily readable by Excel and Google Sheets, 
 * complete with consolidated calculation header metrics, summary groups, 
 * and structural details.
 */
export const exportPremiumSpreadsheet = ({
  transactions,
  baseIncome,
  baseBalance,
  currentCurrency,
  userEmail,
}: ReportData) => {
  const fileDate = new Date().toISOString().split('T')[0];
  const symbol = getCurrencySymbol(currentCurrency);

  const totalInflow = baseIncome + baseBalance;
  const totalSpent = transactions.reduce((sum, t) => sum + t.amount, 0);
  const totalPaid = transactions.reduce((sum, t) => sum + (t.paid_amount || 0), 0);
  const totalPending = totalSpent - totalPaid;
  const netBalance = totalInflow - totalSpent;

  let csvContent = "\uFEFF"; // Byte Order Mark (BOM) strictly required for perfect MS Excel character alignment

  // Section 1: Dashboard Header block
  csvContent += '===================================================\n';
  csvContent += `💰 FINANÇAS PRO - DEMONSTRATIVO FINANCEIRO CONSOLIDADO 💰\n`;
  csvContent += '===================================================\n';
  csvContent += `E-mail do Proprietário: , ${userEmail}\n`;
  csvContent += `Exportação Realizada em: , ${new Date().toLocaleString('pt-BR')}\n`;
  csvContent += `Moeda Padrão de Auditoria: , ${currentCurrency} (${symbol})\n`;
  csvContent += `Volume de Lançamentos Catalogados: , ${transactions.length}\n`;
  csvContent += '\n';

  // Section 2: Financial balance sheet summary of indicators
  csvContent += '---------------------------------------------------\n';
  csvContent += 'INDICADORES CONSOLIDADOS\n';
  csvContent += '---------------------------------------------------\n';
  csvContent += `A. DISPONÍVEL LÍQUIDO TOTAL (SALÁRIO BASE + SALDO BASE): , ${formatCurrency(totalInflow, currentCurrency).replace(',', '.')}\n`;
  csvContent += `B. CUSTO TOTAL DE CONTAS ENTRADAS: , ${formatCurrency(totalSpent, currentCurrency).replace(',', '.')}\n`;
  csvContent += `C. TOTAL PAGO EFETIVAMENTE (BAIXAS): , ${formatCurrency(totalPaid, currentCurrency).replace(',', '.')}\n`;
  csvContent += `D. PASSO PENDENTE (RESTA A ADIMPLIR): , ${formatCurrency(totalPending, currentCurrency).replace(',', '.')}\n`;
  csvContent += `E. SALDO PROJETADO EM CARTEIRA (A - B): , ${formatCurrency(netBalance, currentCurrency).replace(',', '.')}\n`;
  csvContent += `ESTADO DE LIQUIDEZ E CONFORMIDADE: , ${netBalance >= 0 ? "SUPERÁVIT" : "DEFICIT DE CAIXA DE ALERTA"}\n`;
  csvContent += '\n';

  // Section 3: Distribution list by categories
  const catMap: Record<string, number> = {};
  transactions.forEach(t => {
    const c = t.cat || 'Outros';
    catMap[c] = (catMap[c] || 0) + t.amount;
  });

  csvContent += '---------------------------------------------------\n';
  csvContent += 'RESUMO POR CATEGORIA DE CONSUMO\n';
  csvContent += '---------------------------------------------------\n';
  csvContent += 'Categoria de Gasto, Despesa Acumulada\n';
  Object.keys(catMap).forEach(k => {
    const l = k.charAt(0).toUpperCase() + k.slice(1);
    csvContent += `"${l}", "${formatCurrency(catMap[k], currentCurrency)}"\n`;
  });
  csvContent += '\n';

  // Section 4: Pure Database Sheet structure
  csvContent += '---------------------------------------------------\n';
  csvContent += 'RAIZ DETALHADA DE COMPROMISSOS E INVESTIMENTOS\n';
  csvContent += '---------------------------------------------------\n';
  csvContent += 'No.,Identificador ID,Lançamento Financeiro,Valor de Origem,Valor Liquidado,Valor Pendente,Status Geral,Categoria,Vencimento Pactuado,Fluxo Classificação\n';

  transactions.forEach((t, index) => {
    const remaining = t.amount - (t.paid_amount || 0);
    const labelStatus = remaining <= 0 ? 'PAGO INTEGRAL' : t.paid_amount > 0 ? 'PAGO PARCIAL' : 'PENDENTE DE BAIXA';
    const cleanName = t.name.replace(/"/g, '""');
    const displayCat = t.cat ? t.cat.charAt(0).toUpperCase() + t.cat.slice(1) : 'Outros';
    const displayDue = t.due ? t.due.split('-').reverse().join('/') : 'N/A';

    csvContent += `${index + 1},"${t.id}","${cleanName}",${t.amount},${t.paid_amount || 0},${remaining},"${labelStatus}","${displayCat}","${displayDue}","${getTypeLabel(t.type)}"\n`;
  });

  // Safe download trigger
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.setAttribute("href", url);
  link.setAttribute("download", `FinancasPro_Auditoria_Consolidada_${fileDate}.csv`);
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
};

// Internal utility to generate highly random session hash signatures for extreme quality feel
const idxGenerator = (length: number) => {
  let result = '';
  const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  const charactersLength = characters.length;
  for (let i = 0; i < length; i++) {
    result += characters.charAt(Math.floor(Math.random() * charactersLength));
  }
  return result;
};
